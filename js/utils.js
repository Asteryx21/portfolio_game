...
function ItemCollision({ items, player, itemOffset = { x: 0, y: 0 } }) {
...
function showThesis() {
	window.open(
		"https://ieeexplore.ieee.org/abstract/document/10125205",
		"_blank"
	);
}
function showPaper() {
...
function viewGit(git) {
	...
	if (git === 7) {
		window.open(
			"https://github.com/Asteryx21/Natural-language-processing",
			"_blank"
		);
	}
	if (git === 8) {
		window.open(
			"https://github.com/Asteryx21/Knowledge-representation-and-reasoning",
			"_blank"
		);
	}
	if (git === 9) {
